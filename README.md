...
│   ├── src/
│   │   ├── App.tsx            # Главен компонент — единична графика, sidebar, export
│   │   ├── config.ts          # API_BASE от VITE_BACKEND_URL
│   │   ├── utils/
│   │   │   └── market.ts      # fetchMarketSnapshot() — споделен utility
│   │   └── components/
│   │       ├── Navbar.tsx     # Macro статистики, symbol/interval контрол, Експорт
│   │       ├── TradingChart.tsx  # Candlestick + EMA 21/50/200 + canvas overlay за план зони
│   │       └── Sidebar.tsx    # Анализ / Планове / AI Робот tabs + server wake-up
│   ├── vercel.json            # Rewrite: /api/* → Render backend
│   └── vite.config.ts         # Dev proxy: /api → localhost:5000
│
├── backend/                   # Node.js + Express (порт 5000)
│   ├── src/
│   │   ├── index.js           # REST endpoints + WebSocket сървър + Supabase client
│   │   └── python/
...
│   ├── plans/                 # Локален fallback: JSON + Markdown файлове с планове
...
TradingChart показва единична графика. Когато има активен план за избрания символ, canvas overlay рисува цветни правоъгълни зони директно върху графиката:
- **Зелена зона** — entry zone
- **Червена зона** — stop loss
- **Сини зони** — take profit targets (TP1, TP2, TP3)
...
```
GET /api/plans → Supabase DB (таблица "plans")
...
Планове се съхраняват в Supabase (`plans` таблица) с `upsert` при генериране. При изтриване се маха от Supabase и от локалните файлове (ако съществуват).

### 7. Server Wake-Up (Sidebar)

Render изключва безплатния instance след ~15 мин бездействие. При зареждане на Sidebar компонента се изпраща `GET /api/health` ping. Ако сървърът не отговори в рамките на 30 сек, се показва бутон „Събуди сървъра" с polling докато instance-ът не отговори.
...
| POST | `/api/plans/generate` | Генерира AI план (Gemini) + записва в Supabase |
| GET | `/api/plans` | Списък всички планове (от Supabase) |
| GET | `/api/plans/:id/markdown` | Markdown доклад на план (от Supabase) |
| GET | `/api/plans/:id/monitor` | Текущ статус на план |
| DELETE | `/api/plans/:id` | Изтрива план от Supabase + локални файлове |
...
| `SUPABASE_URL` | URL на Supabase проекта (задължително) |
| `SUPABASE_SERVICE_KEY` | Service Role ключ за Supabase (задължително, bypasses RLS) |
